...
c(mean weight) f(0c) sum  h1(Car type (mean weight in lbs.)) h3(nil) ///
title(Table 1: Simple twoway summary table of means) fn(auto.dta)

tabout foreign rep78 using `output'/tab2.docx, replace style(docx) font(bold) ///
...
title(Table 12: Oneway summary table illustrating multiple summary measures) ///
fn(Source: auto.dta) open ptotal(none)

if `doasproject'==1 {
	project, creates("`output'/tab1.xlsx")
	project, creates("`output'/tab2.xlsx")
}
